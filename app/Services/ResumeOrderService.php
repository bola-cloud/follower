...
class ResumeOrderService
{
    ...
    public function getEligibleUsers(Order $order)
    {
        ...
        // Query new eligible users using the same logic as batchCheckEligibility
        // This mirrors batchCheckEligibility: use normalized URL comparisons,
        // extract username from normalized target for profile_link comparison,
        // and exclude users who have done/external on OTHER orders with same normalized URL.
        $eligibleUserIds = DB::table('users')
            ->select('users.id')
            ->where('users.type', 'user')
            // Exclude users who already have done/external actions on THIS order
        ...
        // Combine pending and new eligible users, preserving pending users first
