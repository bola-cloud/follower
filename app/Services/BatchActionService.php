...
class BatchActionService
{
            ...
            foreach ($chunks as $chunkIndex => $chunk) {
                try {
                    ...
                    if ($current > $maxPerMinute) {
                        ...
                        try {
                            Redis::decrby($globalKey, $chunkCount);
                        } catch (\Throwable $__e) {
                        }
                        ...
                        while ($backoffAttempts < $backoffMax) {
                            ...
                            if ($current + $chunkCount <= $maxPerMinute) {
                                try {
                                    Redis::incrby($globalKey, $chunkCount);
                                    Redis::expire($globalKey, 70);
                                } catch (\Throwable $e) {
                                }
                                $delayed = true;
    ...
    private function performBatchInsert(array $batchData, ?int $orderId = null, bool $applySession = true): int
    {
        ...
        try {
            ...
            $sql = "INSERT IGNORE INTO actions (order_id, user_id, type, status, created_at, updated_at, data) VALUES "
