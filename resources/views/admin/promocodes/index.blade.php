...
@section('content')
    <div class="container py-4">
        ...
        <form method="GET" class="form-inline my-3">
            <input type="text" name="search" class="form-control mr-2" placeholder="كود الترويج"
        ...
        <form method="POST" action="{{ route('admin.promocodes.bulkDelete') }}"
            onsubmit="return confirm('هل أنت متأكد من حذف الرموز المحددة؟')">
            @csrf
            @method('DELETE')
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($promo->uses_count >= $promo->max_uses)
                                    <span class="badge badge-danger">مكتمل</span>
                                @endif
                            </td>
                            <td>{{ $promo->activated_at ? \Carbon\Carbon::parse($promo->activated_at)->format('Y-m-d H:i') : '-' }}
                            </td>
                            <td>{{ $promo->created_at->format('Y-m-d H:i') }}</td>
                            <td>
                                <a href="{{ route('admin.promocodes.edit', $promo->id) }}"
                                    class="btn btn-sm btn-primary">تعديل</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center">لا توجد رموز</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
