        ...
        <form method="POST" action="{{ route('admin.settings.update') }}">
            ...
            <div class="row">
                @php
                    ...
                    $numberKeys = ['points_per_follow', 'points_per_like', 'points_per_ads', 'ads_per_user_per_day', 'added_points', 'referral_points', 'points_add_delay'];
                @endphp
                                ...
                                @else
                                    @if(in_array($key, $numberKeys))
                                        <input type="number" name="{{ $key }}" id="{{ $key }}" class="form-control"
                                            value="{{ setting($key) }}" step="1" min="0">
                                    @else
            ...
            <div class="row mt-3">
                <div class="col-md-12 mb-4">
                    <div class="card border-info shadow-sm">
                        <div class="card-body">
                            <label for="preferred_cookie_user_id" class="font-weight-bold mb-2 d-block">اختر حساب الكوكيز
                                المفضل (سيستخدمه النظام أولاً)</label>
                            @php
                            ...
                            <select name="preferred_cookie_user_id" id="preferred_cookie_user_id" class="form-control">
                                ...
                                <option value="" {{ setting('preferred_cookie_user_id') === '' ? 'selected' : '' }}>-- لا شيء
                                    -- (سيتم اختيار حساب عشوائي من المتوفرين)</option>
                            ...
                            <small class="form-text text-muted">اختر هنا حساب واحد سيُستعمل دائماً لطلبات Instagram عندما
                                تكون كوكيز متوفرة.</small>
                        </div>
                    </div>
                </div>
            </div>
